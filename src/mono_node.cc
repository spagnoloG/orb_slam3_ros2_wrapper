...
public:
  ...
  void GrabImage(const sensor_msgs::msg::Image::ConstSharedPtr &msg)
  {
    ...
    Sophus::SE3f Tcc0 = mpSLAM->TrackMonocular(cv_ptr->image,
                         cv_ptr->header.stamp.sec + cv_ptr->header.stamp.nanosec*1e-9);
    Sophus::SE3f Twc = (Tcc0 * Tc0w).inverse();
    rclcpp::Time msg_time(cv_ptr->header.stamp.sec, cv_ptr->header.stamp.nanosec, RCL_ROS_TIME);
...
int main(int argc, char **argv)
{
  ...
  auto image_sub = node->create_subscription<sensor_msgs::msg::Image>(
      "/camera/image_raw", 1, std::bind(&ImageGrabber::GrabImage, &igb, std::placeholders::_1));
