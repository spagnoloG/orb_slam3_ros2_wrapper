...
void setup_ros_publishers(const rclcpp::Node::SharedPtr &node,
  ...
  if (!rpy_rad.isZero(0)) {
    ...
    RCLCPP_INFO(
        node->get_logger(),
        "World frame will be rotated by RPY (in that order) %f %f %f (rad)",
...
void publish_ros_tf_transform(Sophus::SE3f Twc_SE3f,
                              const std::string &frame_id,
                              const std::string &child_frame_id,
                              rclcpp::Time msg_time) {
  auto transform_stamped =
      SE3f_to_transform_stamped(Twc_SE3f, frame_id, child_frame_id, msg_time);
  tf_broadcaster->sendTransform(transform_stamped);
}

void publish_ros_tracked_mappoints(
    std::vector<ORB_SLAM3::MapPoint *> map_points, rclcpp::Time msg_time) {
  sensor_msgs::msg::PointCloud2 cloud =
      tracked_mappoints_to_pointcloud(map_points, msg_time);
  map_points_pub->publish(cloud);
}

geometry_msgs::msg::TransformStamped
SE3f_to_transform_stamped(Sophus::SE3f T_SE3f, const std::string &frame_id,
                          const std::string &child_frame_id,
                          rclcpp::Time stamp) {
