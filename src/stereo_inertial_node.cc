...
public:
  ...
  cv::Mat GetImage(const sensor_msgs::msg::Image::ConstSharedPtr &img_msg)
  {
    ...
    {
      RCLCPP_ERROR(rclcpp::get_logger("Stereo_Inertial"), "cv_bridge exception: %s", e.what());
    }
    ...
    while (rclcpp::ok())
    {
      ...
      if (!imgLeftBuf.empty() && !imgRightBuf.empty() && !mpImuGb->imuBuf.empty())
      {
        ...
        if (fabs(tImLeft - tImRight) > maxTimeDiff)
          continue;
        double latestImuTime = mpImuGb->imuBuf.back()->header.stamp.sec + mpImuGb->imuBuf.back()->header.stamp.nanosec * 1e-9;
        if (tImLeft > latestImuTime)
          continue;

        rclcpp::Time msg_time(imgLeftBuf.front()->header.stamp.sec,
                              imgLeftBuf.front()->header.stamp.nanosec, RCL_ROS_TIME);
        {
          std::lock_guard<std::mutex> lock(mBufMutexLeft);
        ...
        std::vector<ORB_SLAM3::IMU::Point> vImuMeas;
        {
          ...
          while (!mpImuGb->imuBuf.empty() &&
                 (mpImuGb->imuBuf.front()->header.stamp.sec + mpImuGb->imuBuf.front()->header.stamp.nanosec * 1e-9) <= tImLeft)
          {
